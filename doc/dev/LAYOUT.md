...
- **One second past the tombstone.** Beating that one
...
- **A record that comes back adopts the key set it was just described with.**
  Keeping the set it was deleted with would re-advertise tags other devices
...
resolve to the delete, since ties are sticky. That is the clock's limit.
...
**Tag counts are filtered at READ time.** A posting
...
(id, keys, value intact) and the timeline lists it FIRST. Timestamps are NOT
...
SALTING IT WAS TRIED AND REVERTED. Salting with
...
converting anything (`c/ais.c`, mdel_seek): tombstones from the reverted salted
era keep working, and `--forget-deleted` retires them when the user wants.

**A delete takes the file the INDEX made, and never the file it merely points
at.** That line is the whole rule, and the test is ownership:
`blobs/<ts>~<tag>.txt` and `blobs/<ts>~<tag>.aisc` are documents this index
...
states the difference in the same locked words; GUI.md carries them, and the
web-GUI obligations that go with them.
